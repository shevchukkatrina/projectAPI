...
const updateEvent = async (req, res) => {
  try {
    ...
    const isOrganizer =
      user.role === "organizer" && event.organizerId.toString() === userId;
    ...
    if (
      status &&
      !["active", "cancelled", "completed", "postponed"].includes(status)
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (totalTickets !== undefined) {
      ...
      if (newTotalTickets < soldTickets) {
        return res.status(400).json({
          success: false,
          message:
            "Cannot reduce total tickets below the number of sold tickets",
        });
      }
